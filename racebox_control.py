...
class RBControl:
	def __init__(self, fControl: Frame):
		self.fControl = fControl

		#create disposable frame
		f = Frame(self.fControl)
	...
	#	
		...
		#the start countdown button
  		#private method
		#to use hide on the notebook tabs we need to pass in a ref to the notebook
		#will think about that one
		#the alternative is tkraise on a frame - that might also be a solution to show/hide
		...
		startBtn = Button(f, text='Start Countdown', command=__countdownStart)
